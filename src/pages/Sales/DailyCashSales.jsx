...
export default function DailyCashSales() {
  ...
  useEffect(() => {
    async function fetchSales() {
      setLoading(true);
      try {
        const res = await axios.get(`${API_BASE_URL}/api/sales/cash-sales`);
        const sorted = res.data.data.sort((a, b) => new Date(b.date) - new Date(a.date));
        setSales(sorted);
        setFilteredSales(sorted);
      } catch (err) {
        console.error(err);
        setError("Failed to load cash sales data.");
      } finally {
        setLoading(false);
      }
    }
                  ...
                  <tbody className="bg-white divide-y divide-gray-100">
                    {dailySales.length > 0 ? (
                      ...
                      <tr>
                        ...
                        >
                          No cash sales found for this date.
                        </td>
                      </tr>
                    )}
                    {/* Daily Total Row */}
