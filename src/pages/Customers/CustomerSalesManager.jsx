...
// --- MAIN COMPONENT ---
const CustomerSalesManager = ({customerId}) => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            const [salesRes, accountsRes] = await Promise.all([
                axios.get(`${BASE_API_URL}/api/sales/customer-sales/${customerId}`),
    ...
    return (
        <>
            <tr className="hover:bg-gray-50/50 transition-colors">
                <td className="px-6 py-4 text-sm font-bold text-indigo-600">{memo?.date ? format(memo?.date, "Pp") : "-"}</td>
...
export default CustomerSalesManager;