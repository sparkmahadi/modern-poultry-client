...
const ReceiveDueManuallyModal = ({ isOpen, onClose, customerId, customerName, onPaymentSuccess }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            await axios.patch(`${API_BASE_URL}/api/sales/receive-customer-due-manually`, {
                paidAmount: Number(formData.paidAmount),
                paymentAccountId: formData.paymentAccountId,
                customerId: customerId
            });

            toast.success(`Success! Distributed ৳${formData.paidAmount} to dues for ${customerName}`);
            onPaymentSuccess();
            onClose();
        ...
        <div className="fixed inset-0 z-50 bg-gray-900 bg-opacity-70 flex justify-center items-center p-4 backdrop-blur-sm">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8 transform transition-all">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-black text-gray-800">💰 Receive Due Payment</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100 text-center">
                        <p className="text-xs text-indigo-600 font-bold uppercase tracking-wider">Customer Name</p>
                        <p className="text-xl font-black text-indigo-900">{customerName}</p>
                    </div>
...
export default ReceiveDueManuallyModal;